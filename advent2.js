...
for (i = 0; i < boxIDs.length; i++) {
  for (j = i+1; j < boxIDs.length; j++) {
    commonLetterTest = "";
    for (k = 0; k < boxIDs[i].length; k++) {
      if (boxIDs[i][k] == boxIDs[j][k]) {
        commonLetterTest = commonLetterTest + boxIDs[i][k];
      }
    }
    if (commonLetterTest.length > result.maxLength) {
      result.maxLength = commonLetterTest.length;
      result.id1 = boxIDs[i];
      result.id2 = boxIDs[j];
      result.commonLetters = commonLetterTest;
    }
  }
}

document.writeln('Result: ' + result.commonLetters.toString());
document.writeln('IDs: ' + result.id1.toString() + ", "
                  + result.id2.toString());
document.writeln('Length: ' + result.maxLength.toString());
